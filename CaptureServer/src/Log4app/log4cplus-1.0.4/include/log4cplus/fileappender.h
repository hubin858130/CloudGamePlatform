...
namespace log4cplus {
	...
	/**
	* DailyFileAppender extends {@link FileAppender} so that the
	* underlying file is rolled over at a user chosen frequency.
	*
	* <h3>Properties</h3>
	* <p>Properties additional to {@link FileAppender}'s properties:
	...
	* </dl>
	*/
	class LOG4CPLUS_EXPORT DailyFileAppender : public FileAppender {
	public:
		// Ctors
		DailyFileAppender(const log4cplus::tstring& filename,
			DailyRollingFileSchedule schedule = DAILY,
			bool immediateFlush = true);
		DailyFileAppender(const log4cplus::helpers::Properties& properties);
	...
	protected:
		...
		void rollover();
		log4cplus::helpers::Time calculateNextRolloverTime(const log4cplus::helpers::Time& t) const;
		static log4cplus::tstring getFilename(const log4cplus::helpers::Time& t,DailyRollingFileSchedule eschedule,const log4cplus::tstring & basefilename);

		// Data
		DailyRollingFileSchedule schedule;
		log4cplus::tstring prefixFilename;
		log4cplus::helpers::Time nextRolloverTime;
