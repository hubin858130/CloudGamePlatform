...
{
	std::locale::global(locale(""));//将全局区域设为操作系统默认区域,modify by yang
    out.open(LOG4CPLUS_TSTRING_TO_STRING(filename).c_str(), mode);
	std::locale::global(locale("C"));//还原全局区域设定,modify by yang
}
...
///////////////////////////////////////////////////////////////////////////////
// DailyFileAppender ctors and dtor
///////////////////////////////////////////////////////////////////////////////

DailyFileAppender::DailyFileAppender(
	const tstring& filename_, DailyRollingFileSchedule schedule_,
	bool immediateFlush_)
	: FileAppender(getFilename(Time::gettimeofday(),schedule_,filename_), std::ios::app, immediateFlush_)
	,prefixFilename(filename_)
{
	init(schedule_);
}
	...
	: FileAppender(properties, std::ios::app)
{
	...
	prefixFilename = properties.getProperty(LOG4CPLUS_TEXT("File"));

	//close the file of fileappend opened in construction function since file name is not correct 
	out.close();
	out.clear();
	filename = getFilename(Time::gettimeofday(),schedule,prefixFilename);
	//reopen the right file
	open(std::ios::out | std::ios::trunc);
...
void
DailyFileAppender::init(DailyRollingFileSchedule schedule_)
{
	this->schedule = schedule_;
...
// doAppend() which performs the locking
void
DailyFileAppender::append(const spi::InternalLoggingEvent& event)
{
	if(event.getTimestamp() >= nextRolloverTime) {
		rollover();
	}
...
DailyFileAppender::rollover()
{
	...
	{
		// Open a new file, e.g. "log".
		filename = getFilename(now,schedule,prefixFilename);
		open(std::ios::out | std::ios::trunc);
		loglog_opening_result (loglog, out, filename);
...
Time
DailyFileAppender::calculateNextRolloverTime(const Time& t) const
{
	switch(schedule)
	{
	case MONTHLY: 
...
tstring
DailyFileAppender::getFilename(const Time& t,DailyRollingFileSchedule eschedule,const log4cplus::tstring & basefilename)
{
	tchar const * pattern = 0;
	switch (eschedule)
	{
