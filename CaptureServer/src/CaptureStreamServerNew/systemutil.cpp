...
#pragma comment(lib, "ws2_32.lib")
		...
		string GetLocalMacAdress()
		{
			...
			if ((dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen)) == NO_ERROR) {
				...
				{
					string strTmpMac;
					sprintf(cMacAdress,"%02x:%02x:%02x:%02x:%02x:%02x",pAdapter->Address[0],pAdapter->Address[1],pAdapter->Address[2],pAdapter->Address[3],pAdapter->Address[4],pAdapter->Address[5]);
