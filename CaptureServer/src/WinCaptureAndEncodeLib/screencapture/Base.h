...
namespace sc {
  ...
  public:
	...
	virtual long update(bool showCursor) = 0;                                   /* Some capture APIs need to poll for new frames; they can use this. */
    ...
    /* Utils. */
	int setCallback(screencapture_callback callback, mouse_callback mouseCb, void* user);
	virtual int getTextureFrame(ID3D11Texture2D*& pFrameTexture) =0;
  ...
  inline int Base::setCallback(screencapture_callback cb, mouse_callback mouseCb, void* u) {
