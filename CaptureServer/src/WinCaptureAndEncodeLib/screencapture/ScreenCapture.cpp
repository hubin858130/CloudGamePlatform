...
namespace sc {
	...
	ScreenCapture::ScreenCapture(screencapture_callback callback, mouse_callback mouseCb, void* user, int driver)
    :impl(NULL)
  {
    ...
    if (0 != impl->setCallback(callback, mouseCb,user)) {
  ...
  long ScreenCapture::update(bool showCursor) {
  ...
  //get texture
  int ScreenCapture::getTextureFrame(ID3D11Texture2D*& pFrameTexture)
  {
	  int nRet = -1;

	  if (impl != NULL)
	  {
		  nRet = impl->getTextureFrame(pFrameTexture);
	  }
	  return nRet;
  }
