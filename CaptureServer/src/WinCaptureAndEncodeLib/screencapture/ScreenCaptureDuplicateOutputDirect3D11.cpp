#include <sstream> 
#include "ScreenCaptureDuplicateOutputDirect3D11.h"
#include "ScreenCaptureUtilsDirect3D11.h"
...
//	void on_scaled_pixels(void* pMap, int width, int height, void* user);
  ...
  long ScreenCaptureDuplicateOutputDirect3D11::update(bool showCursor) {
    ...
    */
    ...
    hr = duplication->AcquireNextFrame(4, &frame_info, &frame);
    ...
    if (S_OK == hr) 
	{
      ...
      if (S_OK == hr) 
	  {
		  renderer.showCursor(showCursor);
		  updateMouse(&frame_info);
		  renderer.scale(frame_tex);
      }
//	  Sleep(5);//huqb 0x887a0026
    }

#if 1//!defined(NDEBUG)    
    else if (DXGI_ERROR_WAIT_TIMEOUT == hr) {
//huqb      printf("Info: tried to acquire a desktop frame, but we timed out.\n");
		hr = 0x01;
    }
  ...
  */
    ...
    if (0ull != info->PointerShapeBufferSize) {
      ...
      if (DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MONOCHROME == pointer_info.Type) {
               ...
               || DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MASKED_COLOR == pointer_info.Type)
        {
		  ...
		  {
			  mousecallback(out, pointer_info.Height*pointer_info.Pitch);
		  }
#endif          
      }
      else {
  ...
  int ScreenCaptureDuplicateOutputDirect3D11::getTextureFrame(ID3D11Texture2D*& pFrameTexture)
  {
	  int nRet = -1;
	  nRet = renderer.getTextureFrame(pFrameTexture);
	  return nRet;
  }
  ...
  void on_scaled_pixels(uint8_t* pixels, int stride, int width, int height, void* user) {
 // void on_scaled_pixels(void* pTexture, int width, int height, void* user){
	...
	{
		cap->pixel_buffer.plane[0] = (uint8_t*)pixels;
