...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid, GUID profileGuid, int nGopSize, int nBps)
{
    if (!m_hEncoder)
    {
	...
	pIntializeParams->encodeConfig->gopLength = nGopSize;// NVENC_INFINITE_GOPLENGTH;
    ...
    //}
	DWORD dwBitRate = nBps;
	
	pIntializeParams->encodeConfig->rcParams.averageBitRate = dwBitRate;
	pIntializeParams->encodeConfig->rcParams.maxBitRate = 6 * (dwBitRate / 5); // 1.2 times
	pIntializeParams->encodeConfig->rcParams.vbvBufferSize = dwBitRate / (30 / 1);
	pIntializeParams->encodeConfig->rcParams.vbvInitialDelay = 0;

	pIntializeParams->encodeConfig->rcParams.rateControlMode = NV_ENC_PARAMS_RC_VBR;
	pIntializeParams->encodeConfig->rcParams.targetQuality = 25;
